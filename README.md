...
<div align="center">
  ...
  **Bộ gõ tiếng Việt hiện đại cho macOS**
  ...
  [![Version](https://img.shields.io/badge/version-1.2.20-blue.svg)](https://github.com/cudin/aurakey/releases)
  [![Homebrew Cask](https://img.shields.io/homebrew/cask/v/aurakey?label=homebrew%20cask)](https://formulae.brew.sh/cask/aurakey)
...
**Aurakey** được tạo ra để giải quyết triệt để những vấn đề trên!
...
- ⌨️ **Dual Mode**: Hỗ trợ cả CGEvent và Input Method Kit (AurakeyIM)
...
<div align="center">
  <img src="aurakey-panel.png" alt="Aurakey Settings Panel" width="800">
</div>
...
| **AurakeyIM** (Thử nghiệm) | Sử dụng Input Method Kit | Mượt mà hơn trong Terminal, Spotlight, Address Bar |
...
Dịch văn bản ngay trong mọi ứng dụng với phím tắt tùy chỉnh. Aurakey cung cấp **hai hướng dịch**, mỗi hướng có phím tắt riêng và các tùy chọn **hoạt động độc lập**:
...
> 💡 Nếu không chọn text, Aurakey sẽ dịch toàn bộ nội dung trong ô nhập liệu.
...
- ✅ Bật/tắt Aurakey cho từng Input Source cụ thể
- 🔀 Phím tắt chuyển nhanh sang Aurakey/ABC
- 🔔 Tự động phát hiện Input Sources tiếng Việt khác

### 11. Hiệu chỉnh Aurakey Engine theo ứng dụng (Window Title Rules)
...
**Cấu hình:** Settings → Nâng cao → Hiệu chỉnh Aurakey Engine theo ứng dụng

#### Hướng dẫn thêm quy tắc mới

1. Mở **Settings** → **Nâng cao** → **Hiệu chỉnh Aurakey Engine theo ứng dụng**
2. Nhấn **"Thêm quy tắc"**
3. Điền thông tin:
...
| **Thanh công cụ nổi** | Điều khiển nhanh Aurakey tại vị trí con trỏ |
| **Loại trừ ứng dụng** | Tắt Aurakey cho các app cụ thể |
...
Aurakey đã có mặt trên [Homebrew Cask](https://formulae.brew.sh/cask/aurakey). Chỉ cần một lệnh duy nhất:

```bash
brew install --cask aurakey
```

Homebrew sẽ tự động tải, cài đặt Aurakey vào thư mục Applications, và quản lý cập nhật cho bạn.

**Cập nhật lên phiên bản mới:**

```bash
brew upgrade --cask aurakey
```

**Gỡ cài đặt:**

```bash
brew uninstall --cask aurakey
```

> **Lưu ý:** Sau khi cài đặt, bạn vẫn cần cấp quyền Accessibility cho Aurakey:
> **System Settings** → **Privacy & Security** → **Accessibility** → Bật quyền cho Aurakey

### Cài đặt từ Release

1. Tải file `Aurakey.dmg` mới nhất từ [Releases](https://github.com/cudin/aurakey/releases)
2. Mở DMG và kéo Aurakey.app vào thư mục Applications
3. Mở Aurakey từ Applications
4. Cấp quyền Accessibility:
   - **System Settings** → **Privacy & Security** → **Accessibility**
   - Bật quyền cho Aurakey

### Build từ mã nguồn

```bash
# Clone repository
git clone https://github.com/cudin/aurakey.git
cd aurakey/Aurakey

# Build release
./build_release.sh

# Output: Release/Aurakey.app, Release/Aurakey.dmg
```

---

## ⌨️ AurakeyIM - Input Method Kit Mode

AurakeyIM là Input Method sử dụng IMKit của Apple, cung cấp trải nghiệm gõ mượt mà hơn trong các ứng dụng có độ trễ phản hồi thấp hoặc có cơ chế autocomplete như Terminal, Spotlight, Address Bar.

### Bundle Identifiers

| Component | Bundle ID |
|-----------|-----------|
| Aurakey (main app) | `com.tdev.Aurakey` |
| AurakeyIM (input method) | `com.tdev.inputmethod.Aurakey` |
| App Group | `group.com.tdev.aurakey` |
...
| **Phím tắt chuyển nhanh** | Tuỳ chỉnh phím tắt toggle giữa Aurakey và ABC |

### Cài đặt AurakeyIM

1. Mở Aurakey Settings → **Input Sources**
2. Click **"Cài đặt AurakeyIM..."**
3. Copy `AurakeyIM.app` vào `~/Library/Input Methods/`
4. Logout/Login lại
5. Mở **System Settings** → **Keyboard** → **Input Sources**
6. Click **"+"** và thêm **"Aurakey Vietnamese"**

### Quyền truy cập cho AurakeyIM

AurakeyIM cần quyền **Accessibility** để xử lý một số tổ hợp phím đặc biệt (như Ctrl+C trong Terminal):

1. Mở **System Settings** → **Privacy & Security** → **Accessibility**
2. Click **"+"** và thêm `AurakeyIM.app` từ `~/Library/Input Methods/`
3. Bật quyền cho AurakeyIM

> **Lưu ý:** Nếu không cấp quyền Accessibility, AurakeyIM vẫn hoạt động bình thường cho việc gõ tiếng Việt. Quyền này chỉ cần thiết để đảm bảo các phím tắt như Ctrl+C hoạt động đúng khi đang có văn bản đang soạn (marked text).

> **Phím hoàn tác:** AurakeyIM sử dụng phím ESC làm phím hoàn tác mặc định (không thể tùy chỉnh do hạn chế của Input Method Kit). Bấm ESC khi đang gõ từ có dấu tiếng Việt sẽ hoàn tác về dạng không dấu.

### Build AurakeyIM từ mã nguồn

Xem hướng dẫn chi tiết tại [AurakeyIM/README.md](AurakeyIM/README.md)
...
```
Aurakey/
├── Shared/               # Shared code between Aurakey and AurakeyIM
...
├── AurakeyIM/               # Input Method Kit bundle
│   ├── Info.plist        # IMKit configuration
│   ├── main.swift        # Entry point
│   └── AurakeyIMController.swift
...
# Build không AurakeyIM
ENABLE_AURAKEYIM=false ./build_release.sh
...
- ✅ Upload `Aurakey.dmg` và `signature.txt` (cho Sparkle auto-update)
...
| **Input Method Kit** | Native input method support (AurakeyIM) |
...
Aurakey sử dụng **Dual Storage System** để đảm bảo settings không bao giờ bị mất:

1. **Primary Storage**: App Group UserDefaults (`group.com.tdev.inputmethod.Aurakey`)
   - Chia sẻ settings giữa Aurakey và AurakeyIM
...
- ✅ Đồng bộ giữa Aurakey và AurakeyIM

---

## 🙏 Cảm ơn

Aurakey được phát triển dựa trên:
...
- **Issues**: [GitHub Issues](https://github.com/cudin/aurakey/issues)
- **Discussions**: [GitHub Discussions](https://github.com/cudin/aurakey/discussions)

---

<div align="center">
  Made with ❤️ & ☕ by Aurakey Contributors
