...
struct AdvancedSection: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                SettingsGroup(title: "Công cụ điều khiển", color: .purple) {
                    VStack(alignment: .leading, spacing: 12) {
                        // Main toggle to enable toolbar
                        Toggle("Bật thanh công cụ điều khiển Aurakey", isOn: $viewModel.preferences.tempOffToolbarEnabled)
    ...
    private var userDictionarySection: some View {
        ...
        .fileExporter(
            ...
            defaultFilename: "aurakey_user_dictionary.txt"
        ) { result in
