...
struct BackupRestoreSection: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                // Import Section
                SettingsGroup(title: "Khôi phục thiết lập", color: .cyan) {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                        ...
                        HStack(spacing: 6) {
                            Image(systemName: "exclamationmark.triangle.fill")
                                ...
                                .font(.caption)
                            Text("Import sẽ ghi đè toàn bộ thiết lập hiện tại và tự động khởi động lại Aurakey.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    }
                ...
                SettingsGroup(title: "Khôi phục mặc định", color: .cyan) {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                        ...
                        HStack(spacing: 6) {
                            Image(systemName: "exclamationmark.triangle.fill")
                                .foregroundColor(.red)
                                .font(.caption)
                            Text("Xoá toàn bộ cài đặt, macro, từ điển và quy tắc tuỳ chỉnh. Thao tác này không thể hoàn tác.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    }
        ...
        .alert("Khôi phục mặc định?", isPresented: $showResetConfirmation) {
            ...
            Text("Toàn bộ cài đặt, macro, từ điển tuỳ chỉnh và quy tắc sẽ bị xoá. Aurakey sẽ trở về trạng thái như mới cài đặt.\n\nHãy export backup trước nếu bạn muốn giữ lại thiết lập hiện tại.")
        }
    ...
    private func exportSettings() {
        ...
        panel.title = "Export Cài đặt"
        panel.message = "Lưu file thiết lập Aurakey"
    ...
    private func importSettings() {
        ...
        panel.title = "Import Cài đặt"
        panel.message = "Chọn file thiết lập Aurakey để import"
        panel.allowedContentTypes = [.propertyList]
...
struct RestartCountdownView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("Aurakey sẽ tự động khởi động lại sau")
                .font(.subheadline)
                .foregroundColor(.secondary)
