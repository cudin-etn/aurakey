...
struct WindowTitleRulesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Cho phép hiệu chỉnh Aurakey Engine xử lý Tiếng Việt theo từng ứng dụng cụ thể (ví dụ: Google Docs trong Safari)")
                        .font(.caption)
        ...
        .fileExporter(
            ...
            contentType: .json,
            defaultFilename: "aurakey_window_rules.json"
        ) { result in
...
struct AddRuleSheet: View {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if overrideInputSource {
                            if availableInputSources.isEmpty {
                                ...
                                Text("Aurakey sẽ tự động chuyển sang Input Source đã chọn.")
                                    .font(.caption2)
                                    .foregroundColor(.secondary)
                                    .padding(.leading, 20)
                            }
    ...
    private func loadAvailableInputSources() {
        ...
        availableInputSources = InputSourceManager.getAllInputSources().map { (id: $0.id, name: $0.displayName) }
    }
}
...
class WindowTitleRulesViewModel: ObservableObject {
    ...
    private func refreshDetectionOnly() {
        ...
        if let bundleId = detector.getCurrentBundleId() {
            let isAurakey = bundleId == "com.tdev.Aurakey" || bundleId == "com.tdev.Aurakey.debug"
    ...
    func refresh() {
        ...
        // Skip updating if current app is Aurakey itself (user is viewing settings)
        // This keeps the previously detected app info visible
        if let bundleId = detector.getCurrentBundleId() {
            let isAurakey = bundleId == "com.tdev.Aurakey" || bundleId == "com.tdev.Aurakey.debug"
            
            if !isAurakey {
                // Only update when not Aurakey
                currentAppName = NSWorkspace.shared.frontmostApplication?.localizedName ?? bundleId
                currentBundleId = bundleId
                currentWindowTitle = detector.getCachedWindowTitle()
            ...
            // If Aurakey, keep previous values (don't update)
