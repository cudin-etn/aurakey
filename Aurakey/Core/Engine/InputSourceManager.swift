...
//  Aurakey
//
//  Manages macOS Input Sources and auto-enables/disables Aurakey based on configuration
//
...
/// Configuration for auto-enabling Aurakey per input source
struct InputSourceConfig: Codable {
    var enabledInputSources: [String: Bool] = [:]  // Input Source ID -> enabled state

    /// Check if Aurakey should be enabled for a given input source
    func isAurakeyEnabled(for inputSourceID: String) -> Bool {
...
class InputSourceManager {
    ...
    @objc private func inputSourceDidChange(_ notification: Notification) {
        ...
        // For Aurakey itself - always enable Vietnamese
        let shouldEnable: Bool
        if Self.isAurakeyInputSource(newSource) {
            shouldEnable = true
        } else {
            // Check if Aurakey should be enabled for this input source
            shouldEnable = config.isAurakeyEnabled(for: newSource.id)
        }

        debugLogCallback?("Input Source: \(previousSource) → \(newSource.displayName) [Aurakey: \(shouldEnable ? "ON" : "OFF")]")
    ...
    func setEnabled(_ enabled: Bool, for inputSourceID: String) {
        ...
        if currentInputSource?.id == inputSourceID {
            if let source = currentInputSource {
                debugLogCallback?("Input Source '\(source.displayName)' → Aurakey: \(enabled ? "ON" : "OFF") (applied now)")
    ...
    func isEnabled(for inputSourceID: String) -> Bool {
        return config.isAurakeyEnabled(for: inputSourceID)
    }
    ...
    static func getAllInputSources() -> [InputSourceInfo] {
        ...
        // Filter out Aurakey itself from the list
        return uniqueSources.filter { !isAurakeyInputSource($0) }
    }
    ...
    /// Check if an input source is Aurakey itself
    static func isAurakeyInputSource(_ source: InputSourceInfo) -> Bool {
        let lowercaseID = source.id.lowercased()
        let lowercaseName = source.name.lowercased()

        let patterns = ["aurakey"]
...
extension Notification.Name {
    ...
    static let inputSourceDidChange = Notification.Name("Aurakey.inputSourceDidChange")
}
