...
extension String {
    ...
    private static let invalidVietnameseEndingCharacters = CharacterSet(charactersIn: "bdgklrvx")

    private static let englishFinalClusters: Set<String> = [
        "ck", "sk", "nk", "lk", "rk",
        "ct", "ft", "pt", "xt", "lt", "st",
        "lp", "mp", "sp",
        "nd", "ld", "rd",
    ...
    private static let englishInitialClusters: Set<String> = [
    ...
    private static let englishStartPatternClusters: Set<String> = [
        "str", "spr", "scr", "spl", "shr", "thr", "sch", "squ",
        "bl", "cl", "fl", "gl", "pl", "sl",
        "br", "cr", "dr", "fr", "gr", "pr",
        "sc", "sk", "sm", "sn", "sp", "st", "sw", "sh",
    ...
    private static let nonTelexDoubleConsonants: Set<String> = [
        "bb", "ff", "hh", "jj", "ll", "mm", "rr", "ss", "vv", "zz"
    ]

    private static let englishSuffixes: Set<String> = [
        "tion", "sion", "ing", "ed", "ness", "ment", "able", "ible", "ful", "less", "ity", "ous", "ive", "or"
    ]

    private static let englishVowelCombos: Set<String> = [
        "ough", "eigh", "augh", "eau", "iew", "ow", "aw", "ies"
    ]

    private static let englishStartVowelCombos: Set<String> = [
        "ough", "eigh", "augh", "eau", "iew"
    ]

    private static let asciiConsonants: Set<Character> = [
        "b", "c", "d", "f", "g", "h", "j", "k", "l", "m", "n", "p", "q", "r", "s", "t", "v", "w", "x", "y", "z"
    ]

    private static let vietnameseConsonantDigraphStarts: Set<Character> = ["c", "g", "k", "n", "p", "q", "t"]

    private static func isAsciiConsonant(_ char: Character) -> Bool {
        asciiConsonants.contains(char)
    }

    private static func vietnameseClusterLength(_ chars: [Character], at index: Int) -> Int {
        let current = chars[index]
        guard vietnameseConsonantDigraphStarts.contains(current), index + 1 < chars.count else {
            return 0
        }
        ...
        switch (current, next) {
        case ("n", "g"):
            if index + 2 < chars.count, chars[index + 2] == "h" {
                return 3
            }
            return 2
        case ("n", "h"), ("c", "h"), ("t", "h"), ("k", "h"), ("p", "h"), ("t", "r"), ("g", "i"), ("q", "u"):
            return 2
        default:
            return 0
        }
    ...
    private static func containsThreeConsecutiveConsonants(excludingVietnameseClustersIn word: String) -> Bool {
        ...
        while index < chars.count {
            let clusterLength = vietnameseClusterLength(chars, at: index)
            if clusterLength > 0 {
            ...
            if isAsciiConsonant(chars[index]) {
    ...
    private static func hasSilentEnglishPattern(_ word: String) -> Bool {
        word.hasPrefix("kn") || word.hasPrefix("wr") || word.hasPrefix("ps") || word.hasPrefix("pn") ||
        word.hasSuffix("mb") || word.hasSuffix("lm") || word.hasSuffix("gn") || word.hasSuffix("bt")
    }
    ...
    var isDefinitelyEnglish: Bool {
        ...
        if word.count >= 2 {
            if let last = word.last {
                if String(last).rangeOfCharacter(from: Self.invalidVietnameseEndingCharacters) != nil {
                    return true
                }
            }
        }
        ...
        if word.count >= 3 {
            for cluster in Self.englishFinalClusters {
        ...
        if word.count >= 3 {
            for cluster in Self.englishInitialClusters {
        ...
        if word.count >= 3 {
            for dc in Self.nonTelexDoubleConsonants {
                if word.contains(dc) {
                    return true
                }
            }
        ...
        if word.count >= 4 {
            for suffix in Self.englishSuffixes {
        ...
        if Self.containsThreeConsecutiveConsonants(excludingVietnameseClustersIn: word) {
            return true
        }
        ...
        if Self.hasSilentEnglishPattern(word) {
            return true
        }
        ...
        if word.count > 3 {
            for combo in Self.englishVowelCombos {
    ...
    var hasEnglishStartPattern: Bool {
        ...
        if word.count >= 3 {
            for cluster in Self.englishStartPatternClusters {
                if word.hasPrefix(cluster) {
                    return true
                }
            }
        ...
        if word.hasPrefix("kn") || word.hasPrefix("wr") || word.hasPrefix("ps") || word.hasPrefix("pn") {
            return true
        }
        ...
        if word.count > 3 {
            for combo in Self.englishStartVowelCombos {
                if word.contains(combo) {
                    return true
                }
            }
