...
class VNEngine {
    ...
    static func keyCodeToChar(_ keyCode: UInt16) -> Character? {
        ...
        // Keep as switch to avoid allocating a Dictionary on every keystroke.
        switch keyCode {
        case 0x00: return "a"
        case 0x01: return "s"
        case 0x02: return "d"
    ...
    static func uppercaseASCII(_ character: Character) -> Character {
        switch character {
        case "a": return "A"
        case "b": return "B"
        case "c": return "C"
    ...
    var vCustomConsonants: Set<UInt16> = [] {
        didSet {
            if oldValue != vCustomConsonants {
                cachedCustomConsonantChars = nil
            }
        }
    }
    ...
    private func handleMainKey(keyCode: UInt16, isCaps: Bool) {
        ...
        let customConsonantChars: Set<Character>
        if let cached = cachedCustomConsonantChars {
            customConsonantChars = cached
        } else {
            let computed = Set(vCustomConsonants.compactMap { VietnameseData.char(for: $0) })
            cachedCustomConsonantChars = computed
            customConsonantChars = computed
        }
    ...
    private func handleModernMark() {
        ...
        if vowelCount == 3 {
            ...
            // The old Aurakey code incorrectly checked for end consonant, but OpenKey doesn't do that.
            // uo -> mark on 'o'
        ...
        for i in 0..<Int(index) {
            ...
            if (charCode & VNEngine.CHAR_CODE_MASK) != 0 {
                ...
                if let char = keyCodeToCharacter(keyCode) {
                    if isCaps {
                        result.append(Self.uppercaseASCII(char))
                    } else {
                        result.append(char)
                    }
                }
...
extension VNEngine {
    ...
    func undoTyping() -> ProcessResult {
        ...
        for keyData in originalKeys {
            ...
            if let char = keyCodeToCharacter(keyCode) {
                let finalChar = isCaps ? Self.uppercaseASCII(char) : char
    ...
    private func convertHookStateToResult(_ hookState: HookState, currentKeyCode: CGKeyCode?, currentCharacter: Character?, isUppercase: Bool) -> ProcessResult {
        ...
        // 
        // However, in Aurakey's architecture, we need to either:
        ...
        if hookState.code == UInt8(vRestore) || hookState.code == UInt8(vRestoreAndStartNewSession) {
            ...
            if hookState.extCode != 5 {
                if let character = currentCharacter {
                    ...
                    let vnChar = VNCharacter(character: isUppercase ? Self.uppercaseASCII(character) : character)
                    result.newCharacters.append(vnChar)
                }
            }
        }
