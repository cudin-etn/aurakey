...
class KeyboardEventHandler: EventTapManager.EventTapDelegate {
            ...
            for i in 0..<result.backspaceCount {
                if let keyDown = CGEvent(keyboardEventSource: source, virtualKey: VietnameseData.KEY_DELETE, keyDown: true),
                   let keyUp = CGEvent(keyboardEventSource: source, virtualKey: VietnameseData.KEY_DELETE, keyDown: false) {
                    keyDown.setIntegerValueField(.eventSourceUserData, value: kAurakeyEventMarker)
                    keyUp.setIntegerValueField(.eventSourceUserData, value: kAurakeyEventMarker)
    ...
    private func verboseDebug(_ message: @autoclosure () -> String) {
        guard verboseEngineLogging, let debugLogCallback else { return }
    ...
    func processKeyEvent(_ event: CGEvent, type: CGEventType, proxy: CGEventTapProxy) -> CGEvent? {
        ...
        if keyCode == VietnameseData.KEY_DELETE {
            verboseDebug("[\(getTimestamp())] ⌫ BACKSPACE received (keyCode=0x\(String(format: "%02X", VietnameseData.KEY_DELETE)))")
        ...
        if VietnameseData.cursorMovementKeys.contains(keyCode) {
            ...
            verboseDebug("[\(getTimestamp())] \(keyName) Arrow/Nav key received (keyCode=0x\(String(format: "%02X", keyCode)))")
        ...
        if isWordBreakKey(character) {
            ...
            verboseDebug("[\(getTimestamp())] \(keyName) Word-break received (index=\(engine.index), spaceCount=\(engine.spaceCount))")
        ...
        if waitTime > 1.0 {
            verboseDebug("[\(getTimestamp())] ⏱ Waited \(String(format: "%.1f", waitTime))ms for previous injection")
        }
    ...
    private func handleBackspace(event: CGEvent, proxy: CGEventTapProxy) -> CGEvent? {
        ...
        let isEnglishModeWithMacro = !isVietnameseEnabled && macroEnabled && macroInEnglishMode
        if isEnglishModeWithMacro {
            verboseDebug("[\(getTimestamp())] ⌫ Backspace in English+Macro mode - pass through")
        ...
        let waitTime = (CFAbsoluteTimeGetCurrent() - waitStart) * 1000
        if waitTime > 1.0 {
            verboseDebug("[\(getTimestamp())] ⏱ Backspace waited \(String(format: "%.1f", waitTime))ms for injection")
        }
        
        verboseDebug("[\(getTimestamp())] ⌫ Processing backspace (index=\(engine.index), spaceCount=\(engine.spaceCount))")
    ...
    private func isWordBreakKey(_ character: Character) -> Bool {
        // Use centralized logic from VNEngine
    ...
    private func isCurrentAppExcluded() -> Bool {
        ...
        if OverlayAppDetector.shared.isOverlayAppVisible() {
            ...
            // Event-driven probe updates overlay method before current key is handled.
            // Without this, keystrokes could use the stale method from the previous app
            // (e.g., .fast from Chrome instead of .autocomplete for Spotlight).
