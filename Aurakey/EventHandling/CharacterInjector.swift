...
// MARK: - Event Marker
// Used to identify events injected by Aurakey - prevents re-processing by event tap
// This is critical for avoiding race conditions in terminal apps
let kAurakeyEventMarker: Int64 = 0x41555241  // "AURA" in hex
...
class CharacterInjector {
    ...
    private let semaphoreTimeout: DispatchTimeInterval = .seconds(2)
    ...
    private func beginInjection() -> Bool {
        let result = injectionSemaphore.wait(timeout: .now() + semaphoreTimeout)
        if result == .timedOut {
            debugCallback?("    ⚠️ beginInjection semaphore timed out")
            return false
        }
        return true
    }
    ...
    func injectSync(backspaceCount: Int, characters: [VNCharacter], codeTable: CodeTable, proxy: CGEventTapProxy) {
        ...
        let acquired = injectionSemaphore.wait(timeout: .now() + semaphoreTimeout) != .timedOut
        defer { if acquired { injectionSemaphore.signal() } }
        if !acquired {
            debugCallback?("    ⚠️ injectSync semaphore timed out — injecting anyway")
        }
        ...
        for (segmentIndex, segment) in segments.enumerated() {
            ...
            case .text:
                ...
                while offset < utf16.count {
                    ...
                    // Mark as Aurakey-injected event to prevent re-processing by event tap
    ...
    func sendBackspaces(count: Int, codeTable: CodeTable, proxy: CGEventTapProxy) {
        ...
        let acquired = beginInjection()
        defer { if acquired { endInjection() } }
        if !acquired { return }
