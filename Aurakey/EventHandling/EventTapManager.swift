...
class EventTapManager {
    ...
    var verboseEventLogging: Bool = false

    @inline(__always)
    private func verboseDebug(_ message: @autoclosure () -> String) {
        guard verboseEventLogging, let debugLogCallback else { return }
    ...
    private func eventCallback(
        ...
        // even when off-console, otherwise Aurakey will stop working when the user returns.
        ...
        // - Aurakey hotkeys firing in the wrong session
        // - Vietnamese processing injecting text into our (inactive) session
        if !isSessionOnConsole {
            return Unmanaged.passUnretained(event)
        }
        
        // CRITICAL: Skip events injected by Aurakey itself
        ...
        if event.getIntegerValueField(.eventSourceUserData) == kAurakeyEventMarker {
            verboseDebug(" → Skipping Aurakey-injected event (marker detected)")
            verboseDebug("MARKER SKIP: type=\(type.rawValue), keyCode=\(event.getIntegerValueField(.keyboardEventKeycode))")
            return Unmanaged.passUnretained(event)
        }
        ...
        // Since Aurakey only processes keyDown for Vietnamese input, keyUp can bypass safely.
        if type == .keyUp {
            return Unmanaged.passUnretained(event)
        }
        ...
        verboseDebug("EventTapManager.eventCallback: type=\(type.rawValue), delegate=\(delegate != nil)")
        ...
        if let hotkey = toggleHotkey {
            ...
            if hotkey.isModifierOnly {
                if type == .flagsChanged {
                    ...
                    verboseDebug(" → flagsChanged: eventModifiers=\(eventModifiers.rawValue), hotkey.modifiers=\(hotkey.modifiers.rawValue)")
                    ...
                    if hasAllRequiredModifiers {
                        ...
                        if !modifierOnlyState.targetModifiersReached {
                            ...
                            verboseDebug(" → Target modifiers REACHED: \(hotkey.displayString)")
                        }
                        ...
                        if modifierOnlyState.targetModifiersReached && !modifierOnlyState.hasTriggered {
                            ...
                            verboseDebug(" → MODIFIER-ONLY HOTKEY TRIGGERED on release: \(hotkey.displayString)")
                    ...
                    if modifierOnlyState.targetModifiersReached {
                        verboseDebug(" → Key pressed while holding modifiers - canceling modifier-only hotkey")
                        modifierOnlyState.targetModifiersReached = false
                ...
                if type == .keyDown {
                    ...
                    if event.keyCode == hotkey.keyCode && eventModifiers == hotkey.modifiers {
                        verboseDebug(" → TOGGLE HOTKEY DETECTED - consuming event")
        ...
        if let hotkey = toolbarHotkey, type == .keyDown, !isHotkeyRecording {
            let eventModifiers = ModifierFlags(from: event.flags)
            if event.keyCode == hotkey.keyCode && eventModifiers == hotkey.modifiers {
                verboseDebug("  → TOOLBAR HOTKEY DETECTED - consuming event")
            ...
            if hotkey.isModifierOnly {
                ...
                if type == .flagsChanged {
                    ...
                    if hasAllRequiredModifiers {
                        ...
                        if !undoModifierOnlyState.targetModifiersReached {
                            ...
                            verboseDebug(" → Undo target modifiers REACHED: \(hotkey.displayString)")
                        }
                        ...
                        if undoModifierOnlyState.targetModifiersReached && !undoModifierOnlyState.hasTriggered {
                            // Was holding target modifiers, now released - TRIGGER!
                            verboseDebug(" → UNDO MODIFIER-ONLY HOTKEY TRIGGERED on release: \(hotkey.displayString)")
                            ...
                            if let callback = onUndoTypingHotkey, callback() {
                                ...
                                verboseDebug(" → Undo performed successfully")
                            } else {
                                verboseDebug(" → Nothing to undo, pass through")
                            }
                        }
                        // Reset state
                    ...
                    // If user presses any key while holding modifiers, cancel the modifier-only hotkey
                    if undoModifierOnlyState.targetModifiersReached {
                        verboseDebug(" → Key pressed while holding modifiers - canceling undo modifier-only hotkey")
                        undoModifierOnlyState.targetModifiersReached = false
                ...
                if type == .keyDown {
                    ...
                    if event.keyCode == hotkey.keyCode && modifiersMatch {
                        verboseDebug(" → UNDO TYPING HOTKEY DETECTED: \(hotkey.displayString)")
                        // Call callback synchronously and check result
                        if let callback = onUndoTypingHotkey, callback() {
                            verboseDebug(" → Undo performed - consuming event")
        ...
        guard let delegate = delegate else {
            verboseDebug(" → No delegate!")
            return Unmanaged.passUnretained(event)
        }

        verboseDebug(" → Calling shouldProcessEvent...")
        guard delegate.shouldProcessEvent(event, type: type) else {
            verboseDebug(" → shouldProcessEvent returned false")
            return Unmanaged.passUnretained(event)
        }

        verboseDebug(" → Calling processKeyEvent...")
    ...
    ///
    /// Problem: When Aurakey uses `.cghidEventTap` (HID level), the event tap intercepts
    /// ALL keyboard events system-wide, including those from other user sessions.
    /// This causes Aurakey to "steal" keystrokes from other users and inject Vietnamese
